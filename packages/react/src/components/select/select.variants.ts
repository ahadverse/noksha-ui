...
export const selectTriggerVariants = pv({
  base: [
    ...
    'data-[placeholder]:text-(--prism-fg-subtle)',
    '[&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-(--prism-fg-muted)',
  ].join(' '),
  variants: {
...
export const selectContentVariants = pv({
  base: [
    ...
    overlayMotion,
    overlaySideMotion,
    'z-(--prism-z-dropdown) overflow-hidden p-1',
    'outline-none',
    // Published by the size middleware — the list never grows past the space
    // that is actually available below the trigger.
    'max-h-[min(var(--prism-available-height,24rem),24rem)] overflow-y-auto',
  ].join(' '),
  variants: {
    matchTriggerWidth: {
      true: 'w-(--prism-anchor-width) min-w-(--prism-anchor-width)',
      false: 'min-w-(--prism-anchor-width)',
    },
  },
...
export const selectItemVariants = pv({
  base: [
    ...
    'rounded-(--prism-radius-sm) py-1.5 pe-8 ps-2',
    'text-(length:--prism-text-sm) outline-none',
    'transition-colors duration-(--prism-duration-instant)',
    ...
    'focus:bg-(--prism-accent-subtle) focus:text-(--prism-accent-fg)',
...
export const selectIndicatorVariants = pv({
  base: 'absolute end-2 flex items-center text-(--prism-accent-fg)',
});

export const selectGroupLabelVariants = pv({
  base: 'px-2 py-1.5 font-medium text-(--prism-fg-muted) text-xs',
});
